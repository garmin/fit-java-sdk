...
public class BufferEncoder implements MesgListener, MesgDefinitionListener {
     ...
     */
    public BufferEncoder(Fit.ProtocolVersion version, int maxStreamSize) {
    ...
    private void validateStreamSize() {
        if (size() > maxStreamSize) {
            throw new FitRuntimeException("Number of bytes: " + size() + " written to BufferEncoder exceeds maximum limit");
        }
    }
}
