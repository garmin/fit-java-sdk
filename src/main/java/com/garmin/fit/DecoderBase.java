...
abstract class DecoderBase implements MesgSource {
        ...
        for (i = 0; i < componentList.size(); i++) {
            ...
            if (component.fieldNum != Fit.FIELD_NUM_INVALID) {
                ...
                if (componentField.components.size() == 1) {
                    Object nestedRawValue;

                    if (is64BitType(componentField.getType())) {
                        nestedRawValue = applyScaleOffset64(bitsValue, componentField.getType(), component.scale, component.offset, componentField.components.get(0).scale, componentField.components.get(0).offset);
                    } else {
                        nestedRawValue = (((bitsValue / component.scale) - component.offset) + componentField.components.get(0).offset) * componentField.components.get(0).scale;
                    }
                ...
                // componentField is an ordinary field, apply scale and offset as usual
                    ...
                    double compOffset = subField == null ? componentField.offset : subField.offset;
                    double compScale  = subField == null ? componentField.scale  : subField.scale;
                    Object rawValue;
    ...
    protected static boolean is64BitType(int type) {
        return type == Fit.BASE_TYPE_SINT64 || type == Fit.BASE_TYPE_UINT64 || type == Fit.BASE_TYPE_UINT64Z;
    }

    protected static Object applyScaleOffset64(long bitsValue, int fieldType,
                                               double componentScale, double componentOffset,
                                               double fieldScale, double fieldOffset) {
        // Recover the true unsigned value when getBitsValue has sign-extended a bit-pattern >= 2^63.
        boolean isUnsigned = (fieldType == Fit.BASE_TYPE_UINT64 || fieldType == Fit.BASE_TYPE_UINT64Z);

        BigDecimal bd = (isUnsigned && bitsValue < 0)
            ? new BigDecimal(BigInteger.valueOf(bitsValue).add(BigInteger.ONE.shiftLeft(64)))
            : new BigDecimal(bitsValue);
        ...
        return isUnsigned ? bd.toBigInteger() : bd.longValue();
    }
