...
public abstract class FieldBase {
    ...
    public Long getBitsValue(int offset, int bits, boolean signed) {
        ...
        while (bitsInValue < bits) {
            ...
            if (bitsInData > 0) {
                ...
                mask = (bitsInData == 64) ? -1L : (1L << bitsInData) - 1;
        ...
        if (value instanceof Number) {
            ...
            if ((scale != Fit.FIELD_DEFAULT_SCALE) || (offset != Fit.FIELD_DEFAULT_OFFSET)) {
                if (value instanceof BigInteger) {
                    BigDecimal decimalValue = new BigDecimal((BigInteger)value);
                    return decimalValue.divide(BigDecimal.valueOf(scale)).subtract(BigDecimal.valueOf(offset));
                }
    ...
    protected void setValueInternal(int fieldArrayIndex, Object value, SubField subField) {
        ...
        if (value == null) {
            ...
            case Fit.BASE_TYPE_UINT64:
            case Fit.BASE_TYPE_UINT64Z:
                BigDecimal bigDecimalRawValue = ((BigDecimal) value).add(BigDecimal.valueOf(offset)).multiply(BigDecimal.valueOf(scale));

                BigInteger bigIntegerValue = bigDecimalRawValue.toBigInteger();
                values.set(fieldArrayIndex, rangeCorrect(getType(), bigIntegerValue));
                break;
            case Fit.BASE_TYPE_BYTE:
    ...
    public void setRawValue(int fieldArrayIndex, Object rawValue) {
        ...
        // Add a scaled/offset value as the underlying field type rather than double
            ...
            case Fit.BASE_TYPE_UINT64:
            case Fit.BASE_TYPE_UINT64Z:
                values.set(fieldArrayIndex, BigInteger.valueOf(Math.round(((Number)rawValue).doubleValue())));
                break;
            case Fit.BASE_TYPE_STRING:
            ...
            case Fit.BASE_TYPE_UINT64:
            case Fit.BASE_TYPE_UINT64Z:
                values.set(fieldArrayIndex, ((BigDecimal) rawValue).toBigInteger());
                break;
    ...
    public BigDecimal[] getBigDecimalValues() {
        return getBigDecimalValues((SubField)null);
    }

    public BigDecimal[] getBigDecimalValues(int subfieldIndex){
        return getBigDecimalValues(getSubField(subfieldIndex));
    }

    public BigDecimal[] getBigDecimalValues(String subfieldName){
        return getBigDecimalValues(getSubField(subfieldName));
    }

    protected BigDecimal[] getBigDecimalValues(SubField subfield){
        BigDecimal[] rv = new BigDecimal[getNumValues()];
    ...
    public BigDecimal getBigDecimalValue(int fieldArrayIndex) {
        return getBigDecimalValueInternal(fieldArrayIndex, null);
    }

    public BigDecimal getBigDecimalValue(int fieldArrayIndex, int subFieldIndex) {
        return getBigDecimalValueInternal(fieldArrayIndex, getSubField(subFieldIndex));
    }

    public BigDecimal getBigDecimalValue(int fieldArrayIndex, String subFieldName) {
        return getBigDecimalValueInternal(fieldArrayIndex, getSubField(subFieldName));
    }

    protected BigDecimal getBigDecimalValueInternal(int fieldArrayIndex, SubField subField) {
        Object value = getValueInternal(fieldArrayIndex, subField);
        ...
        try {
            ...
            if (value == null) {
                ...
                case Fit.BASE_TYPE_SINT64:
                    ...
                    break;
                case Fit.BASE_TYPE_UINT64:
                    int uint64Size = Fit.baseTypeSizes[getType() & Fit.BASE_TYPE_NUM_MASK];
                    for(int i = 0; i < uint64Size; i++) {
                        data.writeByte( Fit.UINT64_INVALID.shiftRight(8 * (uint64Size - 1 - i)).byteValue());
                    }
                    break;
                case Fit.BASE_TYPE_UINT64Z:
                    int uint64ZSize = Fit.baseTypeSizes[getType() & Fit.BASE_TYPE_NUM_MASK];
                    for(int i = 0; i < uint64ZSize; i++) {
                        data.writeByte( Fit.UINT64Z_INVALID.shiftRight(8 * (uint64ZSize - 1 - i)).byteValue());
                    }
                    break;
                case Fit.BASE_TYPE_STRING:
                    data.writeByte(0);
                ...
                case Fit.BASE_TYPE_UINT64:
                case Fit.BASE_TYPE_UINT64Z: {
                    BigInteger bigIntValue = new BigDecimal(value.toString()).toBigInteger();
                    int size = Fit.baseTypeSizes[getType() & Fit.BASE_TYPE_NUM_MASK];
                    for(int i = 0; i < size; i++) {
                        data.writeByte( bigIntValue.shiftRight(8 * (size - 1 - i)).byteValue());
                    }
                    break;
                }
